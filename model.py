...
class OffsetAdapter(nn.Module):
    ...
    def forward(self, x):
        ...
        BT, L, C = x.size()
        B = BT // T
        # Get number of channels from conv layer for offset-adapter
        Ca = self.conv.in_channels
        H = W = round(math.sqrt(L - 1))
        assert L - 1 == H * W
        x_id = x
...
class CLIP(nn.Module):
    ...
    def forward(self, image, text):
        ...
        # image = image.to(self.visual.conv1.weight.device)
        # text = text.to(self.visual.conv1.weight.device)
